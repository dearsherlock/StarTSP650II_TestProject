...
public class StarIOSDKDotPOSPrinterLineModeActivity extends Activity {
	...
	public void ShowCut(View view) {
		...
		Intent myIntent = new Intent(this, cutDotPrinterActivity.class);
		startActivityFromChild(this, myIntent, 0);
	}
	...
	public void ShowTextFormating(View view) {
		...
		myIntent.putExtra("TEXTFORMATTING", 0);
		startActivityFromChild(this, myIntent, 0);
	...
	public void SampleReceipt(View view) {
		...
		myIntent.putExtra("PRINTERTYPE", 0);
		startActivityFromChild(this, myIntent, 0);
	...
	private void InitializeComponent() { // delete view of some function button for POS Printer Line mode
		...
		titleText.setText("Star Micronics Impact Dot Matrix Printer Samples");
		...
		Button barcodes1DButton = (Button) findViewById(R.id.button_Barcodes1D);
		barcodes1DButton.setVisibility(View.GONE);

		Button barcodes2DButton = (Button) findViewById(R.id.button_BarCodes2D);
		barcodes2DButton.setVisibility(View.GONE);
	...
	private void getPortDiscovery(String interfaceName) {
		...
		new AlertDialog.Builder(this).setIcon(android.R.drawable.checkbox_on_background).setTitle("Please Select IP Address or Input Port Name").setCancelable(false).setView(editPortName).setPositiveButton("OK", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int button) {
				...
				SharedPreferences pref = getSharedPreferences("pref", MODE_PRIVATE);
				Editor editor = pref.edit();
				editor.putString("dot_portName", portNameField.getText().toString());
				editor.commit();
			}
